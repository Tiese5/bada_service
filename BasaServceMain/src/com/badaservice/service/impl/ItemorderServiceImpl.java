...
public class ItemorderServiceImpl implements ItemorderService{
	...
	public void updateItemOrder(ItemOrder ItemOrder) throws Exception {
		try {
			int result = sqlsession.update("ItemOrderMapper.updateItemOrder", ItemOrder);
			if( result == 0) {
				throw new NullPointerException();
			}
		} catch (NullPointerException e) {
			sqlsession.rollback();
			throw new Exception("존재하지 않는 조회에 대한 요청입니다.");
		} catch (Exception e) {
			sqlsession.rollback();
			logger.error(e.getLocalizedMessage());
	...
	public List<ItemOrder> selectOrderSelectList(ItemOrder ItemOrder) throws Exception {
		...
		try {
			// ProfessorMapper.updateProfessorItem 기능을 호출한다.
			// 두번째 파라미터는 저장할 데이터를 담고있는 Beans객체
			result = sqlsession.selectList("ItemOrderMapper.selectOrderSelectList", ItemOrder);
			// 리턴값이 저장된 행의 수
			if (result == null) {
				// 저장된 행이 없다면 강제로 예외를 발생시킨다
				// -->이 예외를 처리 가능한 catch 블록으로 제어가 이동한다.
	...
	public void updateJuItemOrder(ItemOrder ItemOrder) throws Exception {
		try {
			int result = sqlsession.update("ItemOrderMapper.updateJuItemOrder", ItemOrder);
			if( result == 0) {
				throw new NullPointerException();
			}
		} catch (NullPointerException e) {
	...
	public int selectAdminBuyCount(ItemOrder ItemOrder) throws Exception {
		...
		try {
			// ProfessorMapper.updateProfessorItem 기능을 호출한다.
			// 두번째 파라미터는 저장할 데이터를 담고있는 Beans객체
			result = sqlsession.selectOne("ItemOrderMapper.selectAdminBuyCount", ItemOrder);
			// 리턴값이 저장된 행의 수
		} catch (Exception e) {
			// 에러가 발생했으므로 SQL 수행 내역을 되돌림
			logger.error(e.getLocalizedMessage());
